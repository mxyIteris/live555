...
void RTSPServer::RTSPClientConnection::handleCmd_notEnoughBandwidth() {
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
	...
	if (requestIncludedSessionId && clientSession == NULL) {
          //system("echo 1 >> /apps/sessionNotFound");
...
#ifdef MAX_SESSIONS
            if ((numSessions - local554cnt) < MAX_SESSIONS) {
	    clientSession
...
#ifdef MAX_SESSIONS
printf("numsessioni:%d local554cnt:%d\n",numSessions,local554cnt);
	 if ((numSessions-local554cnt) >= MAX_SESSIONS) {
             handleCmd_notEnoughBandwidth();
          }
          else {
             //system("echo 2 >> /apps/sessionNotFound");
	     handleCmd_sessionNotFound();
          }
#else
             //system("echo 3 >> /apps/sessionNotFound");
	     handleCmd_sessionNotFound();
#endif
	} 
#ifdef MAX_SESSIONS
        else if ((numSessions-local554cnt) >= MAX_SESSIONS) {
            handleCmd_notEnoughBandwidth();
        }
#endif
	...
	if (clientSession != NULL) {
	  clientSession->handleCmd_withinSession(this, cmdName, urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
          ...
          //system("echo 4 >> /apps/sessionNotFound");
	  handleCmd_sessionNotFound();
	}
...
#ifdef MAX_SESSIONS
	numSessions++;
    printf ("=== %s:%s() numSessions %d\n", __FILE__, __func__, numSessions);
#endif
}
