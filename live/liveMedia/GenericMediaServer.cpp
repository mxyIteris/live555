...
printf("open from %x, local554cnt:%d\n",clientAddr.sin_addr.s_addr,local554cnt);
  	if(clientAddr.sin_addr.s_addr == inet_addr("192.168.200.2"))
		local554cnt++;
...
void GenericMediaServer::ClientConnection::closeSockets() {
  ...
  envir().taskScheduler().disableBackgroundHandling(fOurSocket);
	 ...
	 ::closeSocket(fOurSocket);
		if(fClientAddr.sin_addr.s_addr == inet_addr("192.168.200.2"))
			local554cnt--;

printf("close from %x, local554cnt:%d\n",fClientAddr.sin_addr.s_addr,local554cnt);
	}
